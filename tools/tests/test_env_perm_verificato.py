"""`deploy.sh` — il permesso di `.env` (H15) si chiede al FILE, non al comando.
...
esattamente ciò che impedisce al fallimento di fermare il deploy. Subito dopo,
in tutti e tre i punti, il successo viene dichiarato: due `ok "… .env 600"` e
un `echo "ENV_OK"`, che è la sonda che il PC legge per decidere se proseguire.
Se il `chmod` fallisce (filesystem read-only, ACL, attributo immutabile,
proprietario diverso, `.env` sostituito da una directory), `.env` resta `644`
**e il deploy dichiara riuscito il setup dei segreti.**
...
def _blocchi() -> list[tuple[str, int, str]]:
    ...
    for nome in _SORGENTI:
        ...
        for i, riga in enumerate(righe):
            if re.search(r"^\s*chmod 600 \.env\b", riga):
                frammento = "\n".join(righe[i:i + 2])
...
def test_i_punti_sono_sei_e_non_solo_quelli_della_issue():
    """Sei, non tre: se ne compare uno nuovo va curato, non contato a parte.
...
def test_il_chmod_zitto_non_passa(tmp_path, indice):
    ...
    assert "ENV_PERM_FALLITO" in res.stdout, (
