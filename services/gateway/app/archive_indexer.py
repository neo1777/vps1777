...
def write_rows(db_path: Union[str, Path], rows: Iterable[tuple], *, batch: int = 500) -> int:
    ...
    try:
        ...
        _ensure_speaker_sistema(conn)    # turni del programma scritti `user` (#293)
        n = 0
        buf: list[tuple] = []
        skip_buf: list[tuple] = []
...
_SPEAKER_NOTI = {
    ...
    "strumento": "tool",
    # Un turno che il PROGRAMMA inietta in un record user: notifiche di task finiti,
    # output di comandi locali, riassunti di compattazione, testi delle skill, messaggi
...
def _ensure_speaker_sistema(conn: sqlite3.Connection) -> int:
    """Porta ai DB già caricati la cura di #293: le righe `user` col testo che
    comincia con una forma del programma diventano `sender='sistema'`,
    `speaker='system'`. Ritorna quante ne ha cambiate; idempotente.

    Nei DB i campi del record (`origin`, `isMeta`) non sono stati conservati: qui
    decide solo la forma del testo, ancorata all'inizio. Un turno che il programma
    marcava solo col campo e con un testo senza forma riconoscibile resta `human`
    finché un re-ingest non lo rilegge dalla fonte: lo dice ARCHIVE.md."""
    have = {r[1] for r in conn.execute("PRAGMA table_info(messages)")}
    if not {"speaker", "sender"} <= have:
        return 0
    da_cambiare = [
        rid for rid, testo in conn.execute(
            "SELECT rowid, content FROM messages WHERE sender='user' AND content<>''")
        if _forma_del_programma(testo or "")]
    for i in range(0, len(da_cambiare), 500):
        pezzo = da_cambiare[i:i + 500]
        conn.execute(
            "UPDATE messages SET sender='sistema', speaker=? WHERE rowid IN (%s)"
            % ",".join("?" * len(pezzo)), (speaker_da_sender("sistema"), *pezzo))
    return len(da_cambiare)
...
# Le FORME dei turni che il programma inietta in un record user, per le versioni di
# Claude Code che non lo dichiarano nel record e per i DB già caricati (dove i campi del
# record non ci sono). Ancorate all'inizio del testo: la stessa forma dentro una frase
# è di chi scrive. Misurate il 26/09/2026 sul primario e sui jsonl del disco. NON ci
# stanno `<command-name>` (uno slash command digitato) né `<bash-input>` (un comando
# lanciato con `!`): sono gesti di chi scrive, e restano suoi.
_FORME_DEL_PROGRAMMA = (
    "<task-notification",
    "<local-command-",            # stdout, stderr e caveat dei comandi locali
...
def _forma_del_programma(testo: str) -> bool:
    """True se il testo comincia con una forma che solo il programma scrive."""
    return testo.lstrip().startswith(_FORME_DEL_PROGRAMMA)


def _turno_del_programma(d: dict, testo: str) -> bool:
    """Un record user con testo è un turno del PROGRAMMA, non di chi scrive?

    Prima il fatto del record: `origin.kind` ('human' = chi scrive; ogni altro
    valore, come 'task-notification', 'peer', 'coordinator', = il programma),
    poi `isMeta` e `isCompactSummary`. Solo se il record non dice niente (le
    versioni che non scrivevano questi campi) decide la forma del testo."""
    origin = d.get("origin")
    kind = origin.get("kind") if isinstance(origin, dict) else None
    if kind:
        return str(kind) != "human"
    if d.get("isMeta") or d.get("isCompactSummary"):
        return True
    return _forma_del_programma(testo)
...
def _iter_claude_code(fh: IO[str], project: str) -> Iterator[RowFull]:
    ...
    for line in fh:
        ...
        if sender == "user" and (d.get("isSidechain") or d.get("parent_tool_use_id")):
            ...
            sender = "strumento"
        elif sender == "user" and blocks.text and _turno_del_programma(d, blocks.text):
            # Un turno che il programma inietta: notifiche, output di comandi locali,
            # compattazioni, skill, altre sessioni (#293). Prima era speaker='human'.
            sender = "sistema"
        ultimo_ts = str(ts)
